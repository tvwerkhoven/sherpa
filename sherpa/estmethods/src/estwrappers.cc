...
static void _get_exception_objects()
{
  ...
  if (NULL == est_error ||
      ...
      NULL == est_maxiter ||
      NULL == est_hitnan) {
    PyObject* est_module = PyImport_AddModule("sherpa.estmethods");
    if (NULL == est_module)
      return;
    ...
    // SMD 3/29/07
    if (NULL == est_error)
      est_error = PyDict_GetItemString(est_dict, "EstMethodError");
    if (NULL == est_hardmin)
      est_hardmin = PyDict_GetItemString(est_dict, "EstHardMin");
    if (NULL == est_hardmax)
      est_hardmax = PyDict_GetItemString(est_dict, "EstHardMax");
    if (NULL == est_newmin)
      est_newmin = PyDict_GetItemString(est_dict, "EstNewMin");
    if (NULL == est_maxiter)
      est_maxiter = PyDict_GetItemString(est_dict, "EstMaxIter");
    if (NULL == est_hitnan)
      est_hitnan = PyDict_GetItemString(est_dict, "EstNaN");
  }
  else
    return;
}
				...
				const est_return_code status)
{
  ...
  case EST_NEWMIN:
    ...
    if (NULL == est_newmin || NULL == est_exception)
      ...
      PyErr_SetString(est_exception, 
		      "new minimum found, restarting error method");
      set_correct_error = EXIT_SUCCESS;
    }
    break;
  ...
  case EST_MAXITER:
    ...
    if (NULL == est_maxiter || NULL == est_exception)
      ...
      PyErr_SetString(est_exception, 
		      "maximum number of iterations in scaling function");
      set_correct_error = EXIT_SUCCESS;
    }
    break;
  ...
  case EST_HITNAN:
    ...
    if (NULL == est_hitnan || NULL == est_exception)
      ...
      PyErr_SetString(est_exception, 
		      "NaN encountered during error method");
      set_correct_error = EXIT_SUCCESS;
    }
    break;
...
static PyObject* _wrap_info_matrix( PyObject* self, PyObject* args )
{
  ...
  if ( EST_SUCCESS != status.status ) { 
    if ( NULL == PyErr_Occurred() )
      _raise_python_error("covariance failed", status);
    Py_DECREF( info_obj );
    return NULL; 
  }
...
static PyObject* _wrap_projection( PyObject* self, PyObject* args )
{
  ...
  if ( EST_SUCCESS != status.status ) { 
    if ( NULL == PyErr_Occurred() )
      _raise_python_error("projection failed", status);
    return NULL;
  }
